...
### Сценарий
Длительные измерения с заметным нелинейным дрейфом прибора.

### Настройки
```bash
python scripts/calibration.py \
  --relative data/high_drift/*.dat \
  --absolute data/absolute.xlsx \
  --output results_high_drift.xlsx \
  --method RLM \
  --drift_degree 2 \
  --calib_degree 1 \
  --logging
...
```
calib_proc.processing: INFO: Starting processing with model_type=RLM, drift_degree=2, calib_degree=1
calib_proc.loading: INFO: Loading 8 CG-6 data files  
calib_proc.loading: INFO: Loaded 156 CG-6 readings
calib_proc.processing: INFO: Processing complete. Generated 3 calibration parameters and 15 ties
```
...
python scripts/calibration.py \
  ...
  --absolute data/absolute_precision.xlsx \
...
|-------|-------------|-------------|-----------------|-----------------|
| 23120527 | 1.0023 | -2.3e-06 | 0.0008 | 1.2e-06 |

**Формула калибровки:**
```
g_истинная = 1.0023 × g_измеренная - 2.3e-06 × g_измеренная²
```
