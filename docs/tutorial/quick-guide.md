...
проект/
├── data/
│   ├── relative/           # Файлы CG-6 (*.dat) или CG-5 (*.txt/*.dat)
│   └── absolute.xlsx       # Опорные значения  
└── результаты/
```

Формат каждого файла (CG-6 или CG-5) определяется автоматически по его заголовку, файлы обоих форматов можно передавать вместе.
...
### Файлы CG-6 (*.dat) и CG-5 (*.txt/*.dat)

Стандартные файлы данных с прибора Scintrex CG-6 или Scintrex CG-5. Формат определяется автоматически по заголовку файла (`CG-6 Survey` / `CG-5 SURVEY`), оба формата содержат:
- Метаданные в заголовке (строки с `/`)
- Таблицу измерений, из которой извлекаются колонки Station, Date, Time, CorrGrav, StdErr:
  - в CG-6 это исходные колонки `Station`, `Date`, `Time`, `CorrGrav`, `StdErr`
  - в CG-5 они формируются из колонок `STATION`, `DATE`, `TIME`, `GRAV.`, `SD.`
